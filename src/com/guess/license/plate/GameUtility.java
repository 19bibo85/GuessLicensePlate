...
public class GameUtility {
	...
	public static ArrayList<Plate> CopyDbToList(ArrayList<Plate> plateList, Context context, String where) {
		...
		NewDatabaseHelper databaseHelper = new NewDatabaseHelper(context);
		...
		if(count > 0){
			...
			while (cursor.moveToNext()) {
				...
				plateList.add(plate);
				////Log.e("GAME_UTILITY", "plate Name - " + name + " Difficulty - " + difficulty + " Language - " + language + " Country - " + country + " ImgID - " + imgID + " Continent -" +continent);
			}
		}
